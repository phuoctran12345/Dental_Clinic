...
public class RegisterInformation extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session == null) {
            response.sendRedirect("register.jsp?error=session_expired");
            return;
        }
        ...
        // Tạo tài khoản trước
        int userId = DoctorDB.registerPatient(email, password);

        if (userId > 0) {
            // Lưu thông tin cá nhân
            boolean success = DoctorDB.savePatientInfo(userId, fullName, phone, dateOfBirth, gender);
