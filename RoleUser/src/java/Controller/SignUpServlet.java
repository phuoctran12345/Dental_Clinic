...
public class SignUpServlet extends HttpServlet {
    ...
    String password = request.getParameter("passwordHash");
     ...
     // In log kiểm tra dữ liệu nhận được từ form
    ...
    if (HospitalDB.isPatientExists(email, phone)) {
        response.sendRedirect("signup.jsp?error=exists");
        return;
    }

    // Gọi DAO để thêm khách hàng mới
    boolean success = HospitalDB.registerPatient(name, email, phone, password);
    if (success) {
