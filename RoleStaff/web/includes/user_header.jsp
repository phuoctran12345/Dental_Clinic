                        ...
                        <div class="auth-buttons">
                            <% User user=(User) session.getAttribute("user"); if (user !=null) { %>
                                <div class="user-avatar" style="display: flex; align-items: center; gap: 10px;">
                                    <form id="uploadForm" action="AvatarServlet" method="post"
                                        enctype="multipart/form-data" style="display: none;">
                                        <input type="file" id="avatarInput" name="avatar" accept="image/*"
                                            style="display: none;" />
                                        <input type="hidden" name="id" value="<%= user.getId()%>" />
                                    </form>
                                    <% Patients patient=(Patients) session.getAttribute("patient"); if (patient !=null)
                                        { %>
                                        <span>Welcome, <%= patient.getFullName()%></span>
                                        <% } else { %>
                                            <span>Welcome, guest</span>
                                            <% } %>
                                                <img id="avatarImg" src="<%= user.getAvatar()%>" alt="Avatar" width="35"
                                                    height="35" style="border-radius: 50%; cursor: pointer;" />
                                ...
                                <div class="notification-icon" onclick="toggleNotifications()">
                                    ...
                                    <% List<Notification> unreadNotifs = (List<Notification>)
                                            request.getAttribute("unreadNotifs");
                                        ...
                                        data-notif-id="<%= notif.getId() %>">
                                            ...
                                            <div class="notification-time">
                                                <%= new java.text.SimpleDateFormat("dd/MM/yyyy
                                                    HH:mm").format(notif.getCreatedAt()) %>
                                            </div>
                                </div>
                                <% } } %>
                    </div>
