...
public class DoctorDAO {
    ...
    public static List<String> getAllSpecialties() {
        ...
        try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            System.out.println("DEBUG getAllDoctors: Executing query: " + sql);

            int count = 0;
            while (rs.next()) {
                count++;
                System.out.println("DEBUG getAllDoctors: Processing row " + count);
                
                try {
                Doctors doctor = new Doctors();
                    doctor.setDoctor_id(rs.getLong("doctor_id"));  // Changed to getLong
                    doctor.setUser_id(rs.getLong("user_id"));      // Changed to getLong
                ...
                list.add(doctor);
                    System.out.println("DEBUG getAllDoctors: Added doctor: " + doctor.getFull_name());
                    
                } catch (Exception rowException) {
                    System.err.println("ERROR processing row " + count + ": " + rowException.getMessage());
                    rowException.printStackTrace();
                }
            }
            
            System.out.println("DEBUG getAllDoctors: Total doctors loaded: " + list.size());
...
public static List<Doctors> getBySpecialty(String specialty) {
    List<Doctors> list = new ArrayList<>();
    ...
    // Tìm kiếm bác sĩ theo tên (keyword) và chuyên khoa
    public List<Doctors> searchDoctors(String keyword, String specialty) {
        List<Doctors> list = new ArrayList<>();
