...
public class PaymentConfirmServlet extends HttpServlet {
    ...
    private void showConfirmPage(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<li><strong>Số tài khoản:</strong> 5529062004</li>");
        out.println("<li><strong>Chủ tài khoản:</strong> TRAN HONG PHUOC</li>");
        ...
        if (currentOrderId != null || currentBillId != null) {
            ...
            out.println("<input type='text' name='orderId' class='form-control' placeholder='ORDER_1749726206435 hoặc BILL_C5C23B92' required>");
            out.println("</div>");
    ...
    private void handlePaymentConfirm(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            if ("success".equals(bill.getPaymentStatus())) {
                out.println("{\"success\": true, \"message\": \"Hóa đơn đã được thanh toán trước đó\", \"status\": \"already_paid\"}");
                return;
            }
            ...
            if (updated) {
                ...
                out.println("{\"success\": true, \"message\": \"Thanh toán đã được xác nhận!\", \"orderId\": \"" + bill.getOrderId() + "\"}");
