...
public class DoctorScheduleServlet extends HttpServlet {
    ...
    private void showPersonalSchedule(HttpServletRequest request, HttpServletResponse response)
        ...
        int doctorId = (int) doctor.getDoctor_id();
        // ✅ FIX: Tạo object DoctorScheduleDAO trước khi gọi method
        DoctorScheduleDAO scheduleDAO = new DoctorScheduleDAO();
        List<DoctorSchedule> schedules = scheduleDAO.getSchedulesByDoctorId(doctorId);
        
        // ✅ FIX: Tạo object DoctorDAO trước khi gọi method getAllDoctors
    ...
    private void showSchedulesByDate(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // ✅ FIX: Tạo object DoctorScheduleDAO trước khi gọi method
            DoctorScheduleDAO scheduleDAO = new DoctorScheduleDAO();
            List<DoctorSchedule> allSchedules = scheduleDAO.getSchedulesByDoctorId(doctorId);
            List<DoctorSchedule> schedules = allSchedules.stream()
                    .filter(schedule -> {
                        java.sql.Date workDate = schedule.getWorkDate();
                        return workDate != null && workDate.equals(sqlDate);
                    })
