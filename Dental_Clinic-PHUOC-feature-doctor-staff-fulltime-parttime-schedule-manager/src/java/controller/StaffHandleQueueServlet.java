...
public class StaffHandleQueueServlet extends HttpServlet {
    ...
    private void handleQueueManagement(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            // ✅ FIX: Lấy lịch hẹn hôm nay và tính toán statistics với thời gian realtime - sử dụng static methods
            ...
            // ✅ FIX: Thử lấy appointments theo ngày sử dụng static method
            List<Appointment> todayAppointments = new ArrayList<>();
            try {
                todayAppointments = AppointmentDAO.getAppointmentsByDateStatic(today);
                System.out.println("📅 Today (" + today + "): Loaded " + todayAppointments.size() + " appointments from getAppointmentsByDateStatic()");
            } catch (Exception e) {
            ...
            for (Appointment apt : appointmentsToUse) {
                try {
                    ...
                    if (patient != null) {
                        ...
                        System.out.println("👤 Patient " + apt.getPatientId() + ": " + patient.getFullName() + " | Phone: " + apt.getPatientPhone());
                    } else {
                        apt.setPatientName("Bệnh nhân " + apt.getPatientId());
                        apt.setPatientPhone("0901234567"); // Số demo
                    }
                    
                    // ✅ FIX: Lấy tên bác sĩ thật từ Doctors table với đúng method
                    ...
                    System.err.println("❌ Error loading info for appointment " + apt.getAppointmentId() + ": " + e.getMessage());
                    apt.setPatientName("Bệnh nhân " + apt.getPatientId());
                    apt.setDoctorName("Bác sĩ " + apt.getDoctorId());
            ...
            System.err.println("❌ CRITICAL ERROR in handleQueueManagement: " + e.getMessage());
            e.printStackTrace();
