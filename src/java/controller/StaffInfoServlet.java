...
protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try (PrintWriter out = response.getWriter()) {
        ...
        Staff staff = new StaffDAO().getStaffByUserId((int) userId);
        if (staff == null) {
            out.println("Không tìm thấy staff!");
        } else {
            out.println("<h1>Thông tin Staff</h1>");
            out.println("<ul>");
            out.println("<li>ID: " + staff.getStaffId() + "</li>");
            out.println("<li>Họ tên: " + staff.getFullName() + "</li>");
            out.println("<li>Số điện thoại: " + staff.getPhone() + "</li>");
            out.println("<li>Địa chỉ: " + staff.getAddress() + "</li>");
            out.println("<li>Ngày sinh: " + staff.getDateOfBirth() + "</li>");
