...
public class DoctorDAO {
    ...
    public static List<String> getAllSpecialties() {
        ...
        List<Doctors> list = new ArrayList<>();
        String sql = "SELECT d.*, u.email FROM Doctors d LEFT JOIN users u ON d.user_id = u.user_id";
        ...
        try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                doctor.setUserEmail(rs.getString("email"));
                list.add(doctor);
            }
    ...
    /**
     * Thêm mới bác sĩ (static method)
     */
    public static boolean insertDoctor(int userId, String fullName, String phone, String address, String dateOfBirth, String gender, String specialty, String licenseNumber) {
        Connection conn = null;
        PreparedStatement ps = null;
        try {
            conn = getConnect();
            if (conn != null) {
                String sql = "INSERT INTO Doctors (user_id, full_name, phone, address, date_of_birth, gender, specialty, license_number, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')";
    ...
    /**
     * Xóa bác sĩ theo user_id (static method)
     */
    public static boolean deleteDoctorByUserId(int userId) {
        Connection conn = null;
        PreparedStatement ps = null;
        try {
            conn = getConnect();
