...
public class AddStaffServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        LOGGER.info("Received POST request with parameters: fullName=" + fullName + ", email=" + email + 
                   ", phone=" + phone + ", role=" + roleParam + ", position=" + position + ", employmentType=" + employmentType);

        // Kiểm tra tham số
        if (fullName == null || fullName.trim().isEmpty() || email == null || email.trim().isEmpty() ||
            phone == null || phone.trim().isEmpty() || roleParam == null || roleParam.trim().isEmpty() ||
        ...
        Connection conn = null; // Sử dụng để kiểm soát transaction (nếu DAO không hỗ trợ)
        try {
            if ("STAFF".equalsIgnoreCase(roleParam)) {
                ...
                staffDAO.addStaff(staff, defaultPassword); // Giả định DAO xử lý transaction
                ...
                doctorDAO.addDoctor(doctor, defaultPassword); // Giả định DAO xử lý transaction
